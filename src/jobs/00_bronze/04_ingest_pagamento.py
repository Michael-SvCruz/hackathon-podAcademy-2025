...
SCRIPT: 04_ingest_pagamento.py
OBJETIVO: Ingestão da camada Landing (Raw) para Bronze — Pagamento.
--------------------------------------------------------------------------------
DESCRIÇÃO TÉCNICA:
Este script lê os dados brutos (Parquet) da base de Pagamento do volume 
de origem (Landing), adiciona metadados de auditoria (data de ingestão e origem) 
e salva em formato Delta Lake na camada Bronze.

ESTRUTURA ESPERADA (Landing):
- Tabela transacional: múltiplas linhas por cliente
...
- Particularidades: versionamento de itens (ver Data Quality Report)
...
PRÓXIMAS CAMADAS:
→ Silver: 01_bronze_silver_pagamento.py
...
DEFAULT_INPUT_PATH = "/Volumes/hackathon_2025/default/source/book_pagamento/dados_pagamento/"
DEFAULT_OUTPUT_PATH = "/Volumes/hackathon_2025/default/bronze/pagamento_delta/"
...
def add_metadata(df):
    ...
    """
    print(">>> [Transform] Adicionando metadados de ingestão (Pagamento)...")
...
def main():
    ...
    # Inicia Sessão Spark
    spark = get_spark_session("Bronze_Ingestion_Pagamento")
    ...
    # =========================================================================
    target_table = "hackathon_2025.default.bronze_pagamento"
    df_bronze.write \
        .mode("overwrite") \
        .option("overwriteSchema", "true") \
    ...
    # =========================================================================
    print("\n" + "="*80)
    print("RELATÓRIO FINAL — Bronze Pagamento")
    print("="*80)
    ...
    print(f"  Próximo passo: Silver (01_bronze_silver_pagamento.py)")
    print("="*80 + "\n")
