# Arquivo: mig_oci/data_upload/scripts/bronze_atraso.py
# Adaptado de src/jobs/00_bronze/05_ingest_atraso.py para OCI Data Flow
# Mudancas: paths OCI, imports flat, F.input_file_name(), sem saveAsTable
"""
--------------------------------------------------------------------------------
PROJETO HACKATHON 2025 - ENGENHARIA DE DADOS
SCRIPT: bronze_atraso.py (OCI Data Flow)
OBJETIVO: Ingestao da camada Landing (Raw) para Bronze - Atraso/Faturamento.
--------------------------------------------------------------------------------
ESTRUTURA ESPERADA (Landing):
- Tabela snapshot: fotografia mensal (DAT_REFERENCIA sempre dia 01)
- Grao: multiplas linhas por CPF (fatura/item/ajuste)
...
- Particularidades: alto volume (31.6M linhas), historico legado ate 2006

ANTI-LEAKAGE:
- Este e um snapshot mensal (nao ha eventos futuros aqui).
- Todas as linhas representam o estado NA DATA DA FOTO (DAT_REFERENCIA).
- Seguro para usar na Gold para aggregacao por safra.
--------------------------------------------------------------------------------
"""
...
# =============================================================================
DEFAULT_INPUT_PATH = f"oci://hackathon-2025-landing-zone@{namespace}/source/atraso/"
DEFAULT_OUTPUT_PATH = f"oci://hackathon-2025-bronze-layer@{namespace}/atraso/"
...
def add_metadata(df):
    ...
    """
    print(">>> [Transform] Adicionando metadados de ingestao (Atraso)...")
...
def main():
    ...
    spark = SparkSession.builder.appName("Bronze_Ingestion_Atraso").getOrCreate()
    ...
    # =========================================================================
    print("\n" + "="*80)
    print("RELATORIO FINAL - Bronze Atraso/Faturamento")
    print("="*80)
    ...
    print(f"  Proximo passo: Silver (bronze_silver_atraso)")
