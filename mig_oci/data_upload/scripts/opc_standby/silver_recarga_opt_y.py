# Arquivo: scripts/opc_standby/silver_recarga_opt_y.py
# OPCAO Y: sem cache, quality check na Silver JA ESCRITA (2 passes no OCI)
# Comparar com silver_recarga.py (baseline 18m25s) e opt_x.py
"""
--------------------------------------------------------------------------------
PROJETO HACKATHON 2025 - ENGENHARIA DE DADOS
SCRIPT: silver_recarga_opt_y.py — OPCAO Y (benchmark)
OBJETIVO: Bronze -> Silver sem cache, quality sobre o arquivo escrito.
--------------------------------------------------------------------------------
DIFERENCA VS PRINCIPAL (silver_recarga.py):
- cache() REMOVIDO — sem pressao de memoria nos executors
- count() REMOVIDO
- Coalesce FIXO em 28
- Quality check le da Silver JA ESCRITA no Object Storage (~3GB)
  → valida o que esta realmente no bucket, nao uma copia em memoria
- 2 actions: write(Bronze→Silver) + agg(Silver escrita)

FLUXO:
  Bronze(4GB OCI) → write → Silver(OCI)
  Silver(3GB OCI) → agg+count → metricas

TRADE-OFF:
  + Sem pressao de memoria (cache pode ser problema em free tier)
  + Quality valida o arquivo real no bucket
  - Le ~3GB a mais do OCI (Silver pos-escrita) vs leitura de cache em memoria
  - Se OCI for mais lento que memoria, este script sera mais lento que Opt-X
--------------------------------------------------------------------------------
"""
...
def main():
    parser = argparse.ArgumentParser(description="ETL Bronze to Silver - OPCAO Y (sem cache)")
    ...
    spark = SparkSession.builder.appName("Silver_Recarga_OptY").getOrCreate()
    ...
    # =========================================================================
    # 3) ESCRITA SILVER (action 1 — le Bronze uma unica vez, sem cache)
    # Sem cache(): sem pressao de memoria. Pipeline e map puro (sem shuffle),
    # entao Bronze e lida exatamente 1x durante a escrita.
    # =========================================================================
    ...
    # Le ~3GB da Silver (tipada, comprimida) em vez do cache em memoria.
    # Beneficio: valida o arquivo REAL no bucket, nao a copia em memoria.
    # =========================================================================
    ...
    print("\n" + "="*80)
    print(">>> [Quality] RELATORIO DE QUALIDADE - SILVER RECARGA (OPT-Y)")
    print("="*80)
    ...
    print("\n" + "="*80)
    print(f"Silver RECARGA concluido — OPCAO Y (sem cache, quality na Silver escrita)")
    print(f"  - Registros:          {count_out}")
    print(f"  - Arquivos de saida:  {NUM_OUTPUT_FILES} (~128MB cada)")
    print(f"  - Actions:            2 (write Bronze→Silver + agg Silver escrita)")
    print(f"  - Dedupe:             removida (Gold groupBy absorve 0.3%)")
    print(f"  - I/O OCI:            ~4GB (Bronze) + ~3GB (Silver qualidade)")
    print("="*80 + "\n")
