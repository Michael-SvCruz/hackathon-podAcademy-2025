...
BYTES_PER_ROW_ESTIMATE = 200   # ~200 bytes/row (muitas features M1/M3/M6)
...
Exemplos de uso:
  ...
  python gold_recarga.py \\
    ...
    # Inicializar Spark
    spark = SparkSession.builder.appName("gold_recarga_optz_sf").getOrCreate()
    ...
    if not args.skip_save:
        ...
        print(f">>> [Otimizacao] repartition({num_output_files}, safra_recarga) — ~{TARGET_FILE_SIZE_MB}MB/arquivo")
        print(f">>> [Otimizacao] {count_gold:,} registros, ~{estimated_size_mb:.0f} MB estimados")

        df_gold.repartition(num_output_files, "safra_recarga") \
            .write \
            .format("delta") \
            .mode("overwrite") \
